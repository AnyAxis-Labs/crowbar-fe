...
export const ProfileDetail = ({ id, userStatistic }: Props) => {
  ...
  const stats = [
    { label: "Revenue", value: toCurrency(revenue, { suffix: " ETH", decimals: 2 }) },
    { label: "Total Deployed", value: toCurrency(totalTokens) },
    {
      ...
      value: bestToken ? (
        <div className="flex items-center gap-1">
          ...
          />
          <p className="text-sm font-semibold text-primary">{bestToken?.tokenName}</p>
        </div>
      ) : (
        "--"
      ),
          ...
          >
            <div className="flex w-full items-start justify-between mb-8 px-4">
              <div className="flex items-center gap-4">
                <UserAvatar name={id} className="w-[64px] h-[64px] md:w-10 md:h-10 rounded-full" />
                <div className="flex flex-col items-start gap-1">
                  <p className="text-app-white font-semibold">{headAddress(id)}</p>
                  <Button
                    onClick={() => copyToClipboard(id)}
                    className="bg-inherit bg-gradient-to-r from-[#BDFE1C]/[0.27] to-[#BDFE1C]/[0.3] md:bg-white/[0.05] gap-2 py-1 px-3 h-auto rounded-full text-sm leading-[20px] text-app-white md:hover:bg-white/[0.1]"
                  >
                    {shortAddress(id)}
            ...
            <div className="w-full md:w-auto grid grid-cols-1 lg:grid-cols-3 gap-[2px] rounded-[16px] overflow-hidden">
              {stats.map((stat, index) => (
                ...
                >
                  <p className="text-sm text-foreground font-normal">{stat.label}</p>
                  <div className="text-primary text-[18px] font-semibold">{stat.value}</div>
                </div>
              ))}
            </div>
          </div>
