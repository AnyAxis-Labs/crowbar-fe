...
onStreamingMessage(StreamingEventName.SubscribeChart, (data: any) => {
	console.log(
		...
		StreamingEventName.SubscribeChart,
		data,
	);
	const parsedData: SocketDataProps =
	...
	subscriptionItem.handlers.forEach((handler: Record<string, any>) =>
	...
	for (const channelString of channelToSubscription.keys()) {
		...
		if (handlerIndex !== -1) {
			...
			if (subscriptionItem.handlers.length === 0) {
				...
				unsubscribeToChannel(
					StreamingEventName.UnSubscribeChart,
					channelString,
				);
